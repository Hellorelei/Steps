...
func _ready() -> void:
	...
	if $AnimatedSprite2D:
		...
		printerr("Attention: Pas d'$AnimatedSprite2D pour le mob, celui-ci sera invisible.")
...
func apply_color_mod_to_sprite(new_color: Color = Color(1.0, 1.0, 1.0, 1.0)) -> void:
	$AnimatedSprite2D.self_modulate = new_color


func change_gravity_scale(new_scale: float = 0.0) -> void:
...
## Appelé lorsqu'un mob est défait, pour le détruire.
func destroy() -> void:
	match type:
		"Cannettes":
